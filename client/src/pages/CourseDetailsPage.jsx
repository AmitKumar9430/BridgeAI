  ...
  return (
    <div className="space-y-4">
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-1">
                    ...
                    {trainer.isPrimary && (
                      <span className="text-[9px] font-bold px-1.5 py-0.2 rounded bg-blue-100 text-blue-800 shrink-0">
                        Lead
                      </span>
                    )}
                  </div>
                  <div className="mt-0.5 flex items-center">
                    <span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-amber-900 bg-amber-100/90 border border-amber-300/80 px-2 py-0.5 rounded-full truncate">
      ...
      <div className="flex gap-6 items-start">
        ...
        {sidebarOpen && (
          <aside className="w-72 lg:w-80 shrink-0 bg-white dark:bg-slate-900 border border-slate-200 rounded-xl shadow-sm overflow-hidden sticky top-4 animate-fadeIn transition-all">
            {/* Top Sidebar Header */}
            <div className="p-3.5 bg-slate-100 dark:bg-slate-800/80 border-b border-slate-200 flex items-center justify-between">
              <span className="text-xs font-bold text-slate-800 dark:text-slate-200 tracking-tight">Modular Course Curriculum</span>
              <button
                onClick={() => setSidebarOpen(false)}
                className="text-slate-400 hover:text-slate-700 dark:text-slate-300 p-1"
                            ...
                            return (
                              <button
                                ...
                                className={`w-full text-left px-3 py-2 rounded-md text-xs transition-colors flex items-center justify-between ${
                                  isActive
                                    ? 'bg-slate-100 text-slate-900 font-bold border-l-4 border-emerald-600 pl-2 shadow-2xs'
            ...
            <div className="p-3 bg-emerald-50 border-t border-emerald-200 flex items-center justify-between">
              <div>
                <span className="text-xs font-bold text-emerald-950 block">Backend Engineering</span>
                <span className="text-[10px] text-emerald-700">Full-Stack AI Architecture</span>
              </div>
              <button
        ...
        <main className={`flex-1 min-w-0 transition-all ${sidebarOpen ? '' : 'w-full'}`}>
          {activeTopic ? (
            <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-xl p-6 sm:p-8 shadow-sm space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 mb-2">
                      <span className="font-bold text-blue-700">{courseData?.course?.title || 'Subject'}</span>
                      <span>&rsaquo;</span>
                      ...
                      {!isStudent && courseData?.course?.trainerName ? (
                        <span className="ml-auto bg-slate-100 text-slate-700 dark:text-slate-300 px-2.5 py-0.5 rounded text-[11px] font-medium border border-slate-200">
                          Concerned Faculty: {courseData.course.trainerName}
                        </span>
                      ) : (
                        <span className="ml-auto inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full border border-slate-200 bg-slate-50 text-slate-700 dark:text-slate-300">
                  ...
                  <div className="flex items-center gap-1.5 text-slate-500">
                    <button
                      ...
                      className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 dark:text-slate-400 transition-colors relative"
                    ...
                    <button
                      ...
                      className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 dark:text-slate-400 transition-colors"
                    ...
                    <button
                      ...
                      className={`p-2 hover:bg-slate-100 rounded-lg transition-colors ${
                        bookmarkedTopics.has(activeTopic.id) ? 'text-amber-600' : 'text-slate-600'
                      }`}
              ...
              {activeTopic.bulletPoints && activeTopic.bulletPoints.length > 0 && (
                <ul className="space-y-2.5 text-sm text-slate-700 pl-5 list-disc marker:text-slate-900 dark:text-white">
              ...
              {activeTopic.visualCard && (
                <div className="bg-[#EAF5EC] border border-[#BDE3C5] rounded-xl p-6 shadow-2xs space-y-4">
                  <div className="flex items-center justify-between border-b border-[#BDE3C5] pb-3">
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white">
                      {activeTopic.visualCard.title}
                    </h3>
                    <span className="text-xs font-bold text-emerald-800 bg-emerald-100/80 px-2.5 py-1 rounded-md border border-emerald-300">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                    ...
                    <div className="md:col-span-8 space-y-3">
                      {activeTopic.visualCard.items.map((item, idx) => (
                        <div
                          key={idx}
                          className="bg-white dark:bg-slate-900 border border-slate-200 rounded-lg p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 shadow-2xs"
                        >
                          <span className="font-extrabold text-emerald-700 text-sm">{item.name}</span>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-xl p-12 text-center text-slate-500">
      ...
      {showDiscussionModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              <button onClick={() => setShowDiscussionModal(false)} className="text-slate-400 hover:text-slate-700 dark:text-slate-300 p-1 rounded hover:bg-slate-100">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleAddComment} className="space-y-2">
              <textarea
                ...
                className="w-full px-3 py-2 text-sm border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none"
              />
      ...
      {showNotesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">Personal Study Notes</h3>
                <p className="text-xs text-slate-500">{activeTopic?.title}</p>
              </div>
              <button onClick={() => setShowNotesModal(false)} className="text-slate-400 hover:text-slate-700 dark:text-slate-300 p-1 rounded hover:bg-slate-100">
                <X className="w-4 h-4" />
              </button>
            </div>
